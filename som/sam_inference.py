...
def build_sam_model(model_name, device='cuda'):
    '''
    build model
    '''
    sam_ckpt = SAM_CKPT
...
@torch.no_grad()
def inference(image_file, model, sam_mode="default", alpha=0.1, resize_to_original=True, label_mode ="1", anno_mode=['Mask', 'Mark'], gt_ann=None, output_mode="binary_mask"):
...
def main(args):
    if args.output_folder is not None:
        if args.model_name == "sam":
            sub_folder = args.model_name + f"_{args.sam_mode}"
        else:
    ...
    if args.model_name == "gt":
        assert args.annotation_file is not None and os.path.isfile(args.annotation_file), "annotation_file is required for gt model"

    gt_ann = None
    if args.annotation_file is None or not os.path.isfile(args.annotation_file):
        ...
        if args.annotation_file.endswith('image_ids.json') or "/ids/" in args.annotation_file:
            image_ids = json.load(open(args.annotation_file))
            ...
            annotations = [json.loads(line) for line in open(args.annotation_file)]
    ...
    model = build_sam_model(args.model_name)
    for image_id, image_file in tqdm(zip(image_ids, image_files), total=len(image_files)):
        ...
        if not args.debug and not args.overwrite and os.path.isfile(output_mask_file):
        ...
        if args.debug:
            output, mask = inference(image_file, model, sam_mode=args.sam_mode, anno_mode=args.anno_mode,
