...
class Visualizer:
    ...
    def draw_binary_mask_with_number(
        ...
        if 'Mask-edge' in anno_mode:
            if not mask.has_holes:
                ...
                rgba = np.zeros(shape2d + (4,), dtype="float32")
                rgba[:, :, :3] = color
                rgba[:, :, 3] = (mask.mask == 1).astype("float32") * 0
                has_valid_segment = True
                self.output.ax.imshow(rgba, extent=(0, self.output.width, self.output.height, 0))
    ...
    def draw_number_with_bb(
        self, bbox, color=None, *, text=None, label_mode='1',anno_mode=['Box']
    ):
        """
        Args:
            binary_mask (ndarray): numpy array of shape (H, W), where H is the image height and
        ...
        if text is not None:
            # lighter_color = tuple([x*0.2 for x in color])
            lighter_color = [1,1,1] # self._change_color_brightness(color, brightness_factor=0.7)
            self._draw_number_in_bb(bbox, text, lighter_color, label_mode)
        return self.output
    ...
    def _draw_number_in_bb(self, bb, text, color, label_mode='1'):
        """
        Find proper places to draw text given a binary mask.
        """
        ...
        self.draw_text(text, (center_x, center_y), color=color)
