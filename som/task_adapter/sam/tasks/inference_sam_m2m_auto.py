...
import cv2
import io
from .automatic_mask_generator import load_sam_mask_generator, SamAutomaticMaskGenerator

metadata = MetadataCatalog.get('coco_2017_train_panoptic')

def inference_sam_m2m_auto(model, image, sam_mode, text_size, label_mode='1', alpha=0.1, anno_mode=['Mask'], output_mode="binary_mask"):
    t = []
    ...
    mask_generator: SamAutomaticMaskGenerator = load_sam_mask_generator(mode=sam_mode, sam_model=model, output_mode=output_mode)
    outputs: List[Dict[str, Any]] = mask_generator.generate(image_ori)
