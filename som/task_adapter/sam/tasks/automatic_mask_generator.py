...
MODES = ['default', 'whole', 'part']

def list_available_modes():
    return MODES

def load_sam_mask_generator(mode, sam_model, **kwargs):
    if mode == 'default' or mode is None:  # default model
        print('Loading SAM model for default region proposal')
        return SamAutomaticMaskGenerator(
            sam_model,
            **kwargs
        )
    elif mode == 'whole':  # default model
        print('Loading SAM model for whole region proposal')
        return SamAutomaticMaskGenerator(
            sam_model,
            points_per_side=32,
            pred_iou_thresh=0.8,
        ...
        print('Loading SAM model for part region proposal')
        return SamAutomaticMaskGenerator(
            sam_model,
            points_per_side=32,
            pred_iou_thresh=0.9,
    ...
    #     )
        ...
        raise ValueError(f'Invalid mode specified {mode} not in {MODES}')
